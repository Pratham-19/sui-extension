...
function App() {
    ...
    return (<main className="flex flex-col items-center my-10 space-y-5 text-lg font-cascadia w-[90vw] mx-auto ">
      ...
      <div className="flex flex-col items-start relative w-[80%]">
        ...
        <VSCodeButton onClick={handleAddBalance} className="w-full">
          Add Gas
          {isLoadingBal && (<Loader className="animate-spin ease-in-out  my-auto ml-2"/>)}
        </VSCodeButton>
      ...
      <VSCodeButton onClick={handleDeploy} className="w-[80%] bg-[#4ba721]">
        Deploy
        {isLoading && (<Loader className="animate-spin ease-in-out  my-auto ml-2"/>)}
      </VSCodeButton>
      ...
      {deployAddr && (<div className="text-center">
          <h2>🎉 Succesfully Deployed 🎉</h2>
          <a href={`https://suiscan.xyz/${envr}/tx/${deployAddr}`} className="text-blue-600 text-sm">
            {deployAddr}
          </a>
        </div>)}
    </main>);
